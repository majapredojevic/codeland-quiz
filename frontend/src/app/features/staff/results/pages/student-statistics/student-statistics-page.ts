...
import { StudentSessionPerformance, StudentStatistics } from '../../data-access/results.models';
import {
  formatPercentage,
  formatResponseTime,
  formatScore,
...
@Component({
  selector: 'clq-student-statistics-page',
  imports: [RouterLink],
  providers: [ResultsApiService],
  templateUrl: './student-statistics-page.html',
  styleUrl: './student-statistics-page.scss',
})
export class StudentStatisticsPage implements OnInit {
  private readonly route = inject(ActivatedRoute);
  private readonly api = inject(ResultsApiService);
  private statsRequestVersion = 0;
  private sessionsRequestVersion = 0;

  protected readonly statistics = signal<StudentStatistics | null>(null);
  protected readonly loading = signal(true);
  protected readonly error = signal<'invalid' | 'not-found' | 'generic' | null>(null);
  protected readonly sessions = signal<StudentSessionPerformance[]>([]);
  ...
  protected changeQuizFilter(event: Event): void {
    if (!(event.target instanceof HTMLSelectElement)) return;
    const value = event.target.value;
    const quizId = value === '' ? null : Number(value);
    if (quizId !== null && (!Number.isSafeInteger(quizId) || quizId < 1)) return;
  ...
  private async loadStatistics(): Promise<void> {
    ...
    this.error.set(null);
    try {
      const statistics = await firstValueFrom(this.api.getStudentStatistics(this.studentId));
  ...
  private async loadSessions(pageIndex: number): Promise<void> {
    ...
    try {
      const response = await firstValueFrom(
        this.api.listStudentSessions(
          this.studentId,
          pageIndex,
          this.sessionsPagination().pageSize,
